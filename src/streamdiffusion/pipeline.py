...
class StreamDiffusion:
    ...
    def unet_step(
        ...
        if mask is not None:
            latent_model_input = torch.cat([x_t_latent_plus_uc, mask, mask_latent], dim=1)
    ...
    def __call__(
        ...
        if mask is not None:
            x_0_pred_out = self.predict_x0_batch(x_t_latent, mask=mask, mask_latent=masked_image_latents)
        else:
